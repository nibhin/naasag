...
const CropSelection = () => {
    ...
    const suitableCrops = {
        ...
        clay: {
            torrid: {
                low: {
                    January: ['Watermelon', 'Peas', 'Chili'],
                    February: ['Pumpkin', 'Mustard', 'Sorghum'],
                    March: ['Sorghum', 'Cabbage', 'Tomato'],
                    April: ['Cabbage', 'Tomato', 'Chili'],
                    May: ['Tomato', 'Eggplant', 'Bell Pepper'],
                    June: ['Eggplant', 'Spinach', 'Peas'],
    ...
    const determineClimateZone = (lat) => {
        if (lat >= -23.5 && lat <= 23.5) {
            return 'torrid'; // Inside the torrid zone
        }
        return 'temperate'; // Outside the torrid zone
    };
    ...
    const getSuitableCrop = () => {
        const climateZone = determineClimateZone(selectedLocation.lat);
        if (soilType && moistureLevel && selectedMonth) {
            return suitableCrops[soilType]?.[climateZone]?.[moistureLevel]?.[selectedMonth] || 'No suitable crop found';
        }
